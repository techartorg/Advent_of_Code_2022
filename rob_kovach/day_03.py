...
ompartments of each rucksack is 16 (p), 38 (L), 42 (P), 22 (v), 20 (t), and 
...
test_input = r'''vJrwpWtwJgWrhcsFMMfFFhFp
jqHRNqRjqzjGDLGLrsFMfFZSrLrFZsSL
PmmdzqPrVvPwwTWBwg
wMqvLMZHhHMvwLHjbvcjnnSBnvTQFn
ttgJtRGJQctTZtZT
...
def part_one(input_):
    score = 0
    sacks = input_.splitlines()

    for sack in sacks:
        c1 = sack[:(len(sack)//2)]
        c2 = sack[(len(sack)//2):]
        common = list(set(c1).intersection(set(c2)))[0]
        score += priority.index(common) + 1
    return score
...
def part_two(input_):
    ...
    for i in range(0, len(sacks), 3):
        badge = set.intersection(set(sacks[i]), set(sacks[i+1]), set(sacks[i+2]))
        p = priority.index(list(badge)[0]) + 1
        score += priority.index(list(badge)[0]) + 1
        i += 2
    return score
